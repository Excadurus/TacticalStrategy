...
public class ExecuteMovementState : CursorState
{
    public ExecuteMovementState(Cursor cursor, CursorStateMachine csm) : base(cursor, csm)
    {
    }
    ...
    {
        if(Input.GetKeyUp(KeyCode.A) && cursor.tile.unit == null)
        {
            cursor.selectedUnit.GetComponent<Light>().enabled = false;
            cursor.selectedUnit.tile = cursor.tile;
            cursor.selectedUnit.FinishMove();
